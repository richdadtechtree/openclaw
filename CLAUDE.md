...
| `test_viewer_list.js` | 요약 속 **"🔍 …반드시 연결해서 봐야 할 5가지" 머리말 뒤 줄바꿈 + 번호 목록** 확인(실제 Chromium). 한 줄에 붙어 온 `…5가지 1. … 2. …` 을 머리말(`.brf-sec`) + 한 항목씩(`.brf-num`, 번호 배지·내어쓰기)으로 나눈다. 머리말은 `N가지/포인트/체크포인트/키워드/정리` 로 끝나야 하고 번호는 **1→2→3 순서대로 이어질 때만** 자른다 → `3.5%`·`1. 5배`·평범한 대화 숫자는 오인 안 함. PC/폰 18항목. `node scripts/test_viewer_list.js` |
