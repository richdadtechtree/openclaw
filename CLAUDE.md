...
| `test_viewer_table.js` | 요약 속 **표**와 앞머리 처리 확인(실제 Chromium, 2026-09-24 실제 슬랙 메시지 사용). 슬랙 표는 메시지 `text` 에 없고 `blocks` 의 table 블록에만 있어 **서버 `stock/app.py` `_merge_tables()`** 가 `\| 칸 \| 칸 \|` 줄로 바꿔 **표 바로 앞 블록의 마지막 줄**(예: WHAT 줄)을 기준으로 글 속 제자리에 넣고(못 찾으면 뒤 블록 첫 줄 앞 → JSON 앞), 뷰어 `tableHtml()` 이 표로 그림(머리칸·숫자 오른쪽 정렬·폰에선 표만 옆으로 밀기). 앞머리 안내문(WHAT 없는 줄)은 `.brf-intro`(사진 칸 없음), 제목 다음 `🔴 …임.`(마침표로 끝나는 문장)은 제목 설명, `:white_check_mark:` 표기도 체크 칸. **동그라미 숫자 `①~⑳` 도 번호 목록**(설명 줄은 밝은 색), **작업 설명 줄**("30쪽 전체를 이미지로 확인함"·"page는 PDF 순서"·"원문 기준으로 수정함")은 숨김(`META_NOTE`). 표가 안 보이면 서버 `~/.openclaw/slack_logs/_blocks_sample.json`(특수 블록 견본) 확인. PC/폰 30항목. `node scripts/test_viewer_table.js` |
