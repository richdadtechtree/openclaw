...
| `pilates_blog.py` | **마이비필라테스 네이버 블로그 글을 하루 한 편씩 슬랙 #심부름(`C0BNB0YRGSY`)으로 배달**. ⚠️ **글을 쓰지 않는다 — AI도 안 부른다.** 글은 **클로드가 미리 써서** `workspace/pilates_posts/day01~60.md` (**60편 = 약 2개월치, 전부 서로 다른 글**) 에 넣어둔 것을 순서대로 꺼내 보낸다(보냄 기록 `workspace/pilates_blog/sent.jsonl`). **한 바퀴를 다 돌면 자동으로 처음부터 다시** 돌아 배달이 끊기지 않고, 재탕 글에는 `🔁 N회차` 안내가 붙는다(새로 넣은 글은 보낸 횟수 0이라 먼저 나간다). 사진 컨셉은 **촬영/포인트/캡션 3줄**로 운동 정보까지 담는다. 보내기 직전 **코드가 규칙을 직접 검사** — 제목 25~35자·특수문자 금지·키워드 앞배치, 본문 1000~1200자, `[사진1~5]`, 메인 키워드 3~4회, 소제목 2~3개, 사진 설명 길이, '완치' 등 의학적 단정. 새 글 10편 이하로 남으면 미리 알린다(그때 클로드에게 더 써달라고 하면 재탕 없이 이어진다). **표준 라이브러리만 사용 + AI 키 불필요** → 모델·게이트웨이가 죽어도 배달은 계속된다. `--dry-run`/`--check`(창고 전체 규칙 검사)/`--list`/`--day`/`--file`/`--reset`. 검증: `python3 scripts/test_pilates_blog.py` (33항목) |
