...
| `pilates_blog.py` | **마이비필라테스 네이버 블로그 글을 하루 한 편씩 슬랙 #심부름(`C0BNB0YRGSY`)으로 배달**. ⚠️ **글을 쓰지 않는다 — AI도 안 부른다.** 글은 **클로드가 미리 써서** `workspace/pilates_posts/day01~30.md` 에 넣어둔 것을 순서대로 꺼내 보낸다(보냄 기록 `workspace/pilates_blog/sent.jsonl`, 같은 글 두 번 안 보냄). 보내기 직전 **코드가 규칙을 직접 검사** — 제목 25~35자·특수문자 금지·키워드 앞배치, 본문 1000~1200자, `[사진1~5]`, 메인 키워드 3~4회, 소제목 2~3개, '완치' 등 의학적 단정. 재고 5편 이하면 메시지에 알리고, **다 떨어지면 글을 지어내지 않고 안내만** 보낸다. **표준 라이브러리만 사용 + AI 키 불필요** → 모델·게이트웨이가 죽어도 배달은 계속된다. `--dry-run`/`--check`(창고 전체 규칙 검사)/`--list`/`--day`/`--file`/`--reset`. 검증: `python3 scripts/test_pilates_blog.py` (28항목) |
