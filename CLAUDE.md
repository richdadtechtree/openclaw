...
| `test_viewer_paper.js` | 다이제스트 **글+지면**(기사 옆에 실린 신문 사진, 누르면 형광펜 켜진 뷰어) 확인(실제 Chromium). ⚠️ 2026-09-24 **`📝 글만` 보기와 전환 버튼은 없앰** — 처음부터 글+지면(폭 1120px). 사진 번호는 **GPT 가 요약할 때 붙이는** `• 사진: 07` 줄(요약은 사용자의 **ChatGPT 슬랙 앱**이 만듦 → 규칙은 ChatGPT 지시문에 넣어야 함. SOUL.md 에도 있음, `07.jpg`·`07, 08` 변형도 인식) 또는 **📌 직접 연결**(이 브라우저 `localStorage` 에만 기억), 번호 없으면 **짐작 안 함**. 슬랙 앱 꼬리표(`다음을 사용하여 보냄 <@U…>`) 제거도 확인 · 형광펜은 여전히 저장 0 · 폰 390px 40항목. `node scripts/test_viewer_paper.js` |
| `test_viewer_font.js` | 다이제스트 **요약 글자 크기 조절** 확인(실제 Chromium). 툴바 오른쪽 `[가− 100% 가+]` 6단계(90~160%), 가운데 숫자=100%로. 요약 글은 전부 `px × --fs` 로 계산돼 제목·카테고리·본문·WHAT 라벨이 **같은 비율**로 커지고 버튼·시간·사진 뷰어는 그대로. 글이 커지면 '글만' 폭도 넓힘(한 줄 길이 유지). 한국어 **낱말 중간 줄바꿈 금지(keep-all)**. `localStorage` `digest.fontScale` 하나만 저장 · 폰 390px 19항목. (글만 보기가 없어져 폭은 늘 1120px) `node scripts/test_viewer_font.js` |
| `test_viewer_list.js` | 요약 속 **"🔍 …반드시 연결해서 봐야 할 5가지" 머리말 뒤 줄바꿈 + 번호 목록** 확인(실제 Chromium). 한 줄에 붙어 온 `…5가지 1. … 2. …` 을 머리말(`.brf-sec`) + 한 항목씩(`.brf-num`, 번호 배지·내어쓰기)으로 나눈다. 머리말은 `N가지/포인트/체크포인트/키워드/정리` 로 끝나야 하고 번호는 **1→2→3 순서대로 이어질 때만** 자른다 → `3.5%`·`1. 5배`·평범한 대화 숫자는 오인 안 함. 번호 아래 **설명 줄은 그 항목에 흐리게 붙이고**(1번만 특별해지던 문제), 목록은 카테고리나 `• WHAT` 앞 새 기사 제목에서만 끝난다. **`📌 …핵심 한 줄`→강조 상자, `✅ …체크할 핵심 주제` 아래 줄→✓ 체크 칸(PC 2열·폰 1열)**, `🔴 반드시 체크 | 제목` 기사·평범한 대화는 오인 안 함. **불릿·콜론 없는 새 형식(`43. 제목 WHAT …`/`WHY …`)도 한 기사로 묶음**(`normalizeKV`, 대문자만·"How to" 오인 안 함). **기사 안 보충 설명 줄(`기사 속 테스트: …`)·출처 줄(`원문 · PDF 21쪽`)도 그 기사 안에**(PDF 쪽=사진 단서). PC/폰 56항목. `node scripts/test_viewer_list.js` |
...
| `test_viewer_hide.js` | 웹 **'🧹 지금까지 것 숨기기'** 확인(실제 Chromium). GPT 요약을 새로 받기 전, 그날 먼저 올라온 요약을 **웹 화면에서만** 한 번에 치운다(슬랙 원본은 그대로). 서버 `stock/app.py` `/slack/hide`·`/slack/unhide` 가 날짜별 '이 시각까지 숨김' 기준을 `~/.openclaw/slack_hidden.json` 에 기록 → 폰·PC 동일, `↩ 숨긴 N개 다시 보기` 로 되돌리기. `/slack/data?all=1` 은 전부. 7항목. `node scripts/test_viewer_hide.js` |
| `test_viewer_table.js` | 요약 속 **표**와 앞머리 처리 확인(실제 Chromium, 2026-09-24 실제 슬랙 메시지 사용). 슬랙 표는 메시지 `text` 에 없고 `blocks` 의 table 블록에만 있어 **서버 `stock/app.py` `_merge_tables()`** 가 `\| 칸 \| 칸 \|` 줄로 바꿔 **표 바로 앞 블록의 마지막 줄**(예: WHAT 줄)을 기준으로 글 속 제자리에 넣고(못 찾으면 뒤 블록 첫 줄 앞 → JSON 앞), 뷰어 `tableHtml()` 이 표로 그림(머리칸·숫자 오른쪽 정렬·폰에선 표만 옆으로 밀기). 앞머리 안내문(WHAT 없는 줄)은 `.brf-intro`(사진 칸 없음), 제목 다음 `🔴 …임.`(마침표로 끝나는 문장)은 제목 설명(**제목 옆에 한 줄로 붙어 와도 🔴부터 줄 바꿈**), `:white_check_mark:` 표기도 체크 칸. **동그라미 숫자 `①~⑳` 도 번호 목록**(머리말 뒤에 한 줄로 붙어 와도 · **`…N가지` 뒤는 무조건 줄바꿈** · 설명·안내문 글씨 밝게), **작업 설명 줄**("30쪽 전체를 이미지로 확인함"·"page는 PDF 순서"·"원문 기준으로 수정함")은 숨김(`META_NOTE`). 표가 안 보이면 서버 `~/.openclaw/slack_logs/_blocks_sample.json`(특수 블록 견본) 확인. PC/폰 40항목. `node scripts/test_viewer_table.js` |
