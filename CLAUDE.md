...
| `test_viewer_mark.js` | 신문 사진 **형광펜** 동작 확인(실제 Chromium). 형광펜은 **일회성** — 새로고침하면 사라지고 **서버·브라우저 저장소에 아무것도 안 남긴다**. 도구 3가지(**직선**=자 댄 듯 수평/수직 보정, **동그라미**=타원, 자유)와 색 3가지(빨강·파랑·노랑), 켜기·긋기·**새로고침 시 소멸**·**요청 0건/저장소 0개**·확대 시 따라붙기·창 열린 동안 유지·장별 분리·되돌리기·지우개(동그라미는 테두리 판정)·모드 중 스와이프 차단·다른 창과 분리 29항목. `node scripts/test_viewer_mark.js` |
