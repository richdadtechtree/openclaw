...
| `test_viewer_mark.js` | 신문 사진 **형광펜** 동작 확인(실제 Chromium). 켜기·긋기·서버저장·**새로고침 후 유지**·확대 시 따라붙기·장별 분리·되돌리기·지우개·모드 중 스와이프 차단·**사람별 분리(다른 브라우저엔 안 보임)**·**코드로 기기 잇기** 22항목. `node scripts/test_viewer_mark.js` (개발 환경 전용) |
