...
| `test_viewer_paper.js` | 다이제스트 **보기 방식 2가지**(`📝 글만`=기존 / `📰 글+지면`=기사 옆에 실린 신문 사진, 누르면 형광펜 켜진 뷰어) 확인(실제 Chromium). 사진 번호는 요약문 `• 사진: 07` 줄 또는 **📌 직접 연결**(이 브라우저 `localStorage` 에만 기억), 번호 없으면 **짐작 안 함**. 형광펜은 여전히 저장 0 · 폰 390px 36항목. `node scripts/test_viewer_paper.js` |
