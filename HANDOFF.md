...
- ✅ **2026-09-17: '원문 링크가 엉뚱한 기사로 가는' 문제 해결** — 링크↔제목 실제 대조.
  ...
  - 📰 **2026-09-23: 보기 방식 2가지 — `📝 글만`(기존) / `📰 글+지면`** (사용자 요청).
    ...
    - **자동 번호**: `workspace/SOUL.md` 에 "사진을 보고 요약했다면 기사 끝에 `• 사진: 07`" 규칙 추가
      (확실하지 않으면 쓰지 말 것). 지금 `[신문요약 n/6]` 메시지는 #gpt 에 '사용자 질문'(bot_id 없음)으로
      찍혀 들어오므로 **어느 경로가 만드는지 확인 필요** — 그 경로의 지시문에도 같은 줄을 넣어야 자동 연결이 된다.
      그 전까지는 📌 로 직접 연결하면 된다.
