...
- ✅ **2026-09-17: '원문 링크가 엉뚱한 기사로 가는' 문제 해결** — 링크↔제목 실제 대조.
  ...
  - 🔍 **2026-09-24: "…봐야 할 5가지" 뒤는 무조건 줄바꿈 + 안내문 글씨 밝게** — 실제 화면에서 머리말과 `①` 이 **한 줄**
    (`…5가지 ① *주택 공급…*`)로 와서, 줄 맨 앞 `①` 만 보던 규칙이 놓쳤다 → 목록 인식 실패 → 전부 흐린 `.brf-intro` 로 그려짐.
    `normalizeKV()` 가 **공백 뒤의 `①~⑳`** 도 `N. ` 으로 바꾸고, `splitListHead()` 는 그림문자로 시작하는 `…N가지` 머리말이면
    뒤에 무엇이 붙든 잘라 줄바꿈(번호면 목록, 아니면 `.brf-listlead` 한 줄). `.brf-intro` 색을 흐린 회색 → 본문색 80%.
    검증: `test_viewer_table.js` 3항목 추가(어두운 화면 스크린샷도 확인) + 뷰어 테스트 11종 통과.
  - 🔴 **2026-09-24: 제목 옆에 붙어 온 🔴 표시는 줄 바꿔 설명으로** — `01. 제목 :red_circle: 오늘 신문에서 …중요한 기사임.` 처럼
    제목과 🔴 설명이 **한 줄**로 오면 제목 전체가 굵게 뭉쳤다. 제목 그리기 직전 `\s🔴`(🟠🟡🟢🔵🟣⭐❗‼ 포함) 위치에서 잘라
    앞=제목, 🔴부터=`.brf-flagnote`(제목 아래 설명). 줄 **맨 앞** `🔴 반드시 체크 | 제목` 머리표는 예전 그대로(배지+제목).
    다음 줄로 따로 온 🔴 설명은 이미 처리됨. 검증: `test_viewer_table.js` 2항목 추가 + 뷰어 테스트 11종 통과.
  - 🗞 **2026-09-24: '📝 글만' 보기 제거 — 항상 '글+지면'** (사용자: "글+지면 같이 보이게만, 글만 기능은 없애죠").
    `viewMode()` 는 늘 "paper", 전환 버튼(`.viewseg`)·`viewSeg()`·`digest.view` 기억값 삭제, `<body class="paper">` 로 첫 화면부터
    넓은 폭(깜빡임 없음). 기사가 없는 날의 "지면을 붙일 브리핑이 없어요" 안내문은 매번 떠서 거슬려 제거(사진이 아직 없는 날 안내는 유지).
    테스트 paper·font·json·match·table 에서 전환 버튼 클릭 제거·기대값 조정, 11종 전부 통과.
