...
- **구성**
  | 파일 | 역할 |
  |---|---|
  | `workspace/pilates_posts/day01~60.md` | **글 창고 60편** (클로드가 작성, 규칙 검사 전부 통과). day01~30 과 day31~60 은 **주제는 겹치되 각도·내용이 전혀 다른 글**(중복 아님) → 약 2개월간 재탕 없음 |
  ...
  | `scripts/test_pilates_blog.py` | 네트워크 없이 도는 검증 33항목 |
- **배달 규칙**: **가장 적게 보낸 글 중 파일 이름이 제일 앞인 것**을 고른다(기록: `workspace/pilates_blog/sent.jsonl`, gitignore).
  - 1바퀴째는 day01 → day02 → … 순서. **한 바퀴를 다 돌면 자동으로 2바퀴째**가 시작돼 **배달이 멈추지 않는다**(2026-09-21 요청 반영).
  - 재탕 글에는 `🔁 N회차 — 전에 보낸 글` 안내가 붙는다. **네이버 중복 문서 위험**이 있으니 도입부·경험 문장을 바꿔 올려야 한다.
  - 새 글을 넣으면 보낸 횟수가 0이라 **새 글이 먼저** 나간다. 새 글 **10편** 이하로 남으면 미리 알린다(열흘 여유 → 재탕 전에 채울 수 있음).
  - 글 파일이 **하나도 없을 때만** 배달을 멈추고 안내만 보낸다(exit 1).
- **사진 컨셉**: `촬영 / 포인트 / 캡션` **3줄 구조**(2026-09-21 요청 반영). 포인트에는 동작 정보와 흔한 실수를 담는다
  (예: "어깨가 귀 쪽으로 들리면 잘못된 호흡"). 설명이 20자 미만이면 점검에서 걸린다.
...
- **서버에서 할 일**
  ...
  ```
- **글이 떨어지면**: 배달은 멈추지 않지만 재탕이 시작된다 → 클로드에게 "필라테스 글 30편 더 써줘" 하면
  `workspace/pilates_posts/` 에 day31~ 로 추가되고, 새 글이 먼저 나간다.
...
- **✅ 서버 실배달 확인 완료 (2026-09-21)**: `day01` 이 `#심부름` 에 정상 도착. 창고 60편 전부 점검 통과.
- **✅ cron 등록 완료 (2026-09-21)**: 서버가 KST라 `0 6 * * *` 그대로. 다음 배달은 day02.
  - ⚠️ 등록 당시 venv 가 켜져 있어 cron 에 `~/newspaper/.venv/bin/python3` 이 박혔다. 이 스크립트는
    표준 라이브러리만 쓰므로 venv 가 불필요하고, 그 폴더가 없어지면 조용히 실패한다.
    → `setup-pilates-cron.sh` 가 이제 **시스템 파이썬(`/usr/bin/python3`)을 기본**으로 쓰도록 수정됨.
    서버에서 한 번 다시 실행해 cron 줄을 갱신할 것.
- **남은 확인**: `scripts/setup-pilates-cron.sh` 로 06:00 KST 등록(아직 안 함).
