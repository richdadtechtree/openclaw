...
- **필수 포함 분야: 부동산 · 금융 · 증권 — 각 분야에서 최소 1건씩 반드시 넣는다.**
  (5건 중 3건은 이 세 분야로 채우고, 나머지 2건은 그날 시장 영향도가 높은 기사로 자유 선택)
  - 해당 분야 기사가 그날 정말 없으면 `[<분야> 주요기사 없음]` 한 줄로 표기하고 다른 분야 기사로 대체한다.
  - 각 기사 제목 앞 [카테고리]에 `부동산`/`금융`/`증권` 을 명시한다.
